...
function analyzeContentQuality(chatBotResponse, chatgptAnswer, question) {
  ...
  const analysis = {
    ...
    chatBotHasDirectAnswer: cleanChatBotText.length > 100 && !cleanChatBotText.includes('http'),
    chatgptHasDirectAnswer: chatgptText.length > 100,
    chatBotContent: cleanChatBotText.substring(0, 300),
    chatgptContent: chatgptText.substring(0, 300),
    qualityComparison: 'unknown'
  };
  
  // Determine quality comparison
  if (analysis.chatBotHasDirectAnswer && analysis.chatgptHasDirectAnswer) {
    // Both have direct answers - compare content quality
    if (cleanChatBotText.length > chatgptText.length * 0.8 && cleanChatBotText.length < chatgptText.length * 1.5) {
      analysis.qualityComparison = 'both_good';
    } else if (cleanChatBotText.length > chatgptText.length * 1.5) {
      analysis.qualityComparison = 'chatbot_verbose';
    } else if (cleanChatBotText.length < chatgptText.length * 0.8) {
      analysis.qualityComparison = 'chatgpt_better';
    } else {
      analysis.qualityComparison = 'both_good';
    }
  } else if (analysis.chatBotHasDirectAnswer && !analysis.chatgptHasDirectAnswer) {
...
console.log('Analyzing the correctness and quality of ChatBot vs ChatGPT responses\n');
...
results.forEach(result => {
  ...
  console.log(`ChatBot Has Direct Answer: ${analysis.chatBotHasDirectAnswer}`);
  console.log(`ChatGPT Has Direct Answer: ${analysis.chatgptHasDirectAnswer}`);
  console.log(`Quality Comparison: ${analysis.qualityComparison}`);
  
  if (analysis.qualityComparison === 'chatgpt_better') {
    console.log(`\n❌ ChatBot Issue: ${analysis.chatBotContent.substring(0, 150)}...`);
    console.log(`✅ ChatGPT Better: ${analysis.chatgptContent.substring(0, 150)}...`);
  } else if (analysis.qualityComparison === 'chatbot_better') {
    console.log(`\n✅ ChatBot Better: ${analysis.chatBotContent.substring(0, 150)}...`);
    console.log(`❌ ChatGPT Issue: ${analysis.chatgptContent.substring(0, 150)}...`);
  } else if (analysis.qualityComparison === 'chatbot_verbose') {
    console.log(`\n⚠️  ChatBot Verbose: ${analysis.chatBotContent.substring(0, 150)}...`);
    console.log(`✅ ChatGPT Concise: ${analysis.chatgptContent.substring(0, 150)}...`);
  }
});
...
console.log('\n🎯 KEY FINDINGS:');
console.log('1. Questions where ChatGPT provides better direct answers');
console.log('2. Questions where ChatBot provides better responses');
console.log('3. Questions where ChatBot is too verbose');
console.log('4. Overall content quality patterns');

// Save detailed analysis
const analysisPath = path.resolve(__dirname, "results", `content-quality-analysis-${Date.now()}.json`);
