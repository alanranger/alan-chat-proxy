...
export function extractStructuredDataFromHTML(html) {
  ...
  try {
    ...
    // 1. Participants extraction - look for "Participants: Max X" pattern
    const participantsMatch = allText.match(/participants?\s*:?\s*max\s*(\d+)/i);
    ...
    // 4. Location Address extraction - look for "Location:" pattern
    const locationMatch = allText.match(/location\s*:?\s*([^:\n\r]{1,100}?)(?:\s*\n|\s*$)/i);
    if (locationMatch) {
      extracted.location_address = locationMatch[1].trim();
    }
    
    // 5. Time Schedule extraction - look for "Time:" pattern
    const timeMatch = allText.match(/time\s*:?\s*([^:\n\r]{1,50}?)(?:\s*\n|\s*$)/i);
    if (timeMatch) {
      extracted.time_schedule = timeMatch[1].trim();
    }
    
    // 6. Fitness Level extraction - look for "Fitness:" pattern (handles both inline and multi-line)
    const fitnessMatch = allText.match(/fitness\s*:?\s*(?:([^\n\r]{1,50}?)(?:\s*\n|\s*$)|(?:\s*\n\s*([^\n\r]{1,50}?)(?:\s*\n|\s*$)))/i);
    if (fitnessMatch) {
    ...
    // 7. What to Bring extraction - look for "What to Bring:" pattern
    const whatToBringMatch = allText.match(/what\s*to\s*bring\s*:?\s*([^:\n\r]{1,200}?)(?:\s*\n|\s*$)/i);
    if (whatToBringMatch) {
      let whatToBringText = whatToBringMatch[1].trim();
    ...
    // 8. Course Duration extraction - look for "Duration:" pattern
    const durationMatch = allText.match(/duration\s*:?\s*([^:\n\r]{1,50}?)(?:\s*\n|\s*$)/i);
    if (durationMatch) {
      extracted.course_duration = durationMatch[1].trim();
    }
    
    // 9. Instructor Info extraction - look for "Instructor:" pattern
    const instructorMatch = allText.match(/instructor\s*:?\s*([^:\n\r]{1,100}?)(?:\s*\n|\s*$)/i);
    if (instructorMatch) {
      extracted.instructor_info = instructorMatch[1].trim();
    }
    
    // 10. Availability Status extraction - look for "Availability:" pattern
    const availabilityMatch = allText.match(/availability\s*:?\s*([^:\n\r]{1,50}?)(?:\s*\n|\s*$)/i);
    if (availabilityMatch) {
      extracted.availability_status = availabilityMatch[1].trim();
    }
