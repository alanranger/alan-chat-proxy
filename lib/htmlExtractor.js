...
export function enhanceDescriptionWithStructuredData(originalDescription, structuredData) {
  ...
  // Clean and add original description if it exists
  ...
  if (structuredData.equipmentNeeded) {
    parts.push(`Equipment Needed: ${cleanHTMLText(structuredData.equipmentNeeded)}`);
  }
  
  if (structuredData.experienceLevel) {
    parts.push(`Experience Level: ${cleanHTMLText(structuredData.experienceLevel)}`);
  }
  
  if (structuredData.participants) {
    parts.push(`Participants: ${cleanHTMLText(structuredData.participants)}`);
  }
  
  if (structuredData.location) {
    parts.push(`Location: ${cleanHTMLText(structuredData.location)}`);
  }
  
  if (structuredData.time) {
    parts.push(`Time: ${cleanHTMLText(structuredData.time)}`);
  }
  
  if (structuredData.fitness) {
    parts.push(`Fitness: ${cleanHTMLText(structuredData.fitness)}`);
  }
