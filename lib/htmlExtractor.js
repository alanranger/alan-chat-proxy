...
export function extractStructuredDataFromHTML(html) {
  ...
  try {
    ...
    // Format 5: "Location: North Devon - Hartland Quay" (inline) - stop at next field
    if (!extracted.location_address) {
      const locationInlineMatch = allText.match(/location\s*:\s*([^:*]+?)(?=\s*\*|\s*Duration|\s*Participants:|\s*Time:|\s*Experience|\s*Equipment|\s*$)/i);
