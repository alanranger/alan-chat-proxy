...
export function extractStructuredDataFromHTML(html) {
  ...
  try {
    ...
    // 4. Location Address extraction - look for "Location:" pattern (handles multi-line)
    const locationMatch = allText.match(/location\s*:?\s*(?:([^\n\r]{1,100}?)(?:\s*\n|\s*$)|(?:\s*\n\s*([^\n\r]{1,100}?)(?:\s*\n|\s*$)))/i);
    if (locationMatch) {
      extracted.location_address = (locationMatch[1] || locationMatch[2] || '').trim();
    }
    
    // 5. Time Schedule extraction - look for "Time:" pattern (handles multi-line)
    ...
    // 7. What to Bring extraction - look for "What to Bring:" pattern (handles multi-line)
    ...
    // 8. Course Duration extraction - look for "Duration:" pattern (handles multi-line)
    const durationMatch = allText.match(/duration\s*:?\s*(?:([^\n\r]{1,50}?)(?:\s*\n|\s*$)|(?:\s*\n\s*([^\n\r]{1,50}?)(?:\s*\n|\s*$)))/i);
    if (durationMatch) {
      extracted.course_duration = (durationMatch[1] || durationMatch[2] || '').trim();
    }
    
    // 9. Instructor Info extraction - look for "Instructor:" pattern (handles multi-line)
    const instructorMatch = allText.match(/instructor\s*:?\s*(?:([^\n\r]{1,100}?)(?:\s*\n|\s*$)|(?:\s*\n\s*([^\n\r]{1,100}?)(?:\s*\n|\s*$)))/i);
    if (instructorMatch) {
      extracted.instructor_info = (instructorMatch[1] || instructorMatch[2] || '').trim();
    }
    
    // 10. Availability Status extraction - look for "Availability:" pattern (handles multi-line)
