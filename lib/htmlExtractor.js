...
export function extractStructuredDataFromHTML(html) {
  ...
  try {
    ...
    const locationHtmlMatch = allText.match(/location\s*:\s*([^:]+?)(?=\s*dates?:)/i);
    ...
    // Format 5: "Location: North Devon - Hartland Quay" (inline)
    if (!extracted.location_address) {
    ...
    const fitnessHtmlMatch = allText.match(/fitness\s*:\s*([^:]+?)(?=\s*photography\s*workshop\s*event\s*details?:)/i);
    if (fitnessHtmlMatch) {
