...
export function extractStructuredDataFromHTML(html) {
  ...
  try {
    ...
    // 4. Location Address extraction - look for "* Location:" pattern (handles asterisk-separated fields)
    ...
    if (!extracted.location_address) {
      const locationFallbackMatch = allText.match(/location\s*:\s*([^*\n]+?)(?:\s*\*|\s*\n|\s*$)/i);
      if (locationFallbackMatch) {
        extracted.location_address = locationFallbackMatch[1].trim();
      }
    }
    ...
    // 6. Fitness Level extraction - look for "* Fitness:" pattern (handles asterisk-separated fields)
    ...
    if (!extracted.fitness_level) {
      const fitnessFallbackMatch = allText.match(/fitness\s*:\s*([^*\n]+?)(?:\s*\*|\s*\n|\s*$)/i);
      if (fitnessFallbackMatch) {
        extracted.fitness_level = fitnessFallbackMatch[1].trim();
      }
    }
