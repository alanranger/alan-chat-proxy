...
export function extractStructuredDataFromHTML(html) {
  ...
  try {
    ...
    // 4. Location Address extraction - look for "Location:" pattern (handles multi-line, avoids JS code)
    const locationMatch = allText.match(/location\s*:?\s*(?:([a-zA-Z0-9\s\-\.\/]{1,100}?)(?:\s*\n|\s*$)|(?:\s*\n\s*([a-zA-Z0-9\s\-\.\/]{1,100}?)(?:\s*\n|\s*$)))/i);
    if (locationMatch) {
    ...
    // 6. Fitness Level extraction - look for "Fitness:" pattern (handles both inline and multi-line, avoids JS code)
    const fitnessMatch = allText.match(/fitness\s*:?\s*(?:([a-zA-Z0-9\s\-\.]{1,50}?)(?:\s*\n|\s*$)|(?:\s*\n\s*([a-zA-Z0-9\s\-\.]{1,50}?)(?:\s*\n|\s*$)))/i);
    if (fitnessMatch) {
