...
export function extractStructuredDataFromHTML(html) {
  ...
  try {
    ...
    // 4. Location Address extraction - handle concatenated format
    // Format 1: "Location:North Devon - Hartland QuayDates:" (concatenated, stop at next field)
    ...
    // Format 3: "* Location: North Devon - Hartland Quay *" (asterisk-separated)
    if (!extracted.location_address) {
    ...
    // Format 4: "Location: North Devon - Hartland Quay" (inline)
    if (!extracted.location_address) {
      const locationInlineMatch = allText.match(/location\s*:\s*([^\n]+)/i);
    ...
    // 6. Fitness Level extraction - handle concatenated format
    // Format 1: "Fitness: 2. ModeratePhotography Workshop Event Details:" (concatenated, stop at next field)
    ...
    // Format 3: "* Fitness: 2. Moderate *" (asterisk-separated)
    if (!extracted.fitness_level) {
    ...
    // Format 4: "Fitness: 2. Moderate" (inline)
    if (!extracted.fitness_level) {
