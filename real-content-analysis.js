...
const resultsPath = path.resolve(__dirname, "results", "baseline-comparison-1761343010800.json");
const results = JSON.parse(fs.readFileSync(resultsPath, 'utf8'));

// ChatGPT answers for comparison
...
function analyzeContentQuality(chatBotResponse, chatgptAnswer, question) {
  const chatBotText = chatBotResponse.fullResponse || '';
  const chatgptText = chatgptAnswer || '';
  
  // Remove the greeting message from ChatBot response
  const greetingPattern = /Hi! I can help with workshops, tuition, technical photography questions, equipment recommendations, and course information\. Ask me anything about Alan's services or photography techniques!/;
  const cleanChatBotText = chatBotText.replace(greetingPattern, '').trim();
  
  // Analyze content quality based on actual content, not just length
  const analysis = {
    question,
    chatBotLength: cleanChatBotText.length,
    chatgptLength: chatgptText.length,
    chatBotContent: cleanChatBotText.substring(0, 500),
    chatgptContent: chatgptText.substring(0, 500),
    qualityComparison: 'unknown',
    chatBotIssues: [],
    chatgptStrengths: [],
    contentAnalysis: ''
  };
  
  // Analyze ChatBot content quality
  if (cleanChatBotText.includes('http') && !cleanChatBotText.includes('exposure') && !cleanChatBotText.includes('aperture') && !cleanChatBotText.includes('shutter')) {
    analysis.chatBotIssues.push('Returns article links instead of direct answer');
  }
  
  if (cleanChatBotText.includes('autumn') && !question.includes('autumn')) {
    analysis.chatBotIssues.push('Returns irrelevant autumn photography content');
  }
  
  if (cleanChatBotText.includes('UV filter') && !question.includes('filter')) {
    analysis.chatBotIssues.push('Returns irrelevant UV filter content');
  }
  
  if (cleanChatBotText.includes('bluebell') && !question.includes('bluebell')) {
    analysis.chatBotIssues.push('Returns irrelevant bluebell content');
  }
  
  if (cleanChatBotText.includes('street photography') && !question.includes('street')) {
    analysis.chatBotIssues.push('Returns irrelevant street photography content');
  }
  
  if (cleanChatBotText.length > 2000 && chatgptText.length < 500) {
    analysis.chatBotIssues.push('Extremely verbose compared to ChatGPT');
  }
  
  if (cleanChatBotText.length < 100) {
    analysis.chatBotIssues.push('Response too short to be helpful');
  }
  
  // Analyze ChatGPT content quality
  if (chatgptText.length > 100 && chatgptText.length < 1000) {
    analysis.chatgptStrengths.push('Appropriate length for the question');
  }
  
  if (chatgptText.includes('photography') || chatgptText.includes('camera') || chatgptText.includes('photo')) {
    analysis.chatgptStrengths.push('Relevant to photography');
  }
  
  if (chatgptText.includes('aperture') || chatgptText.includes('shutter') || chatgptText.includes('ISO') || chatgptText.includes('exposure')) {
    analysis.chatgptStrengths.push('Includes technical photography terms');
  }
  
  if (chatgptText.includes('technique') || chatgptText.includes('practice') || chatgptText.includes('learn')) {
    analysis.chatgptStrengths.push('Provides actionable advice');
  }
  
  // Determine quality comparison based on actual content analysis
  if (analysis.chatBotIssues.length > 0 && analysis.chatgptStrengths.length > 0) {
    analysis.qualityComparison = 'chatgpt_better';
  } else if (analysis.chatBotIssues.length === 0 && analysis.chatgptStrengths.length > 0) {
    analysis.qualityComparison = 'both_good';
  } else if (analysis.chatBotIssues.length > 0 && analysis.chatgptStrengths.length === 0) {
    analysis.qualityComparison = 'both_poor';
  } else {
    analysis.qualityComparison = 'chatbot_better';
  }
  
  // Generate content analysis
  if (analysis.chatBotIssues.length > 0) {
    analysis.contentAnalysis = `ChatBot Issues: ${analysis.chatBotIssues.join(', ')}`;
  } else if (analysis.chatgptStrengths.length > 0) {
    analysis.contentAnalysis = `ChatGPT Strengths: ${analysis.chatgptStrengths.join(', ')}`;
  } else {
    analysis.contentAnalysis = 'Both responses are comparable';
  }
  
  return analysis;
}

console.log('🔍 REAL CONTENT QUALITY ANALYSIS');
console.log('='.repeat(80));
console.log('Analyzing the actual content quality of ChatBot vs ChatGPT responses\n');

const qualityAnalyses = [];

results.forEach(result => {
  const analysis = analyzeContentQuality(result.chatBot, result.chatgpt.answer, result.question);
  qualityAnalyses.push(analysis);
  
  console.log(`\n📊 QUESTION: "${result.question}"`);
  console.log('-'.repeat(60));
  console.log(`ChatBot Response: ${analysis.chatBotLength} chars`);
  console.log(`ChatGPT Response: ${analysis.chatgptLength} chars`);
  console.log(`Quality Comparison: ${analysis.qualityComparison}`);
  
  if (analysis.chatBotIssues.length > 0) {
    console.log(`\n❌ ChatBot Issues: ${analysis.chatBotIssues.join(', ')}`);
  }
  
  if (analysis.chatgptStrengths.length > 0) {
    console.log(`\n✅ ChatGPT Strengths: ${analysis.chatgptStrengths.join(', ')}`);
  }
  
  console.log(`\n📝 Content Analysis: ${analysis.contentAnalysis}`);
  
  if (analysis.qualityComparison === 'chatgpt_better') {
    console.log(`\n🔍 ChatBot Content: ${analysis.chatBotContent.substring(0, 200)}...`);
    console.log(`🔍 ChatGPT Content: ${analysis.chatgptContent.substring(0, 200)}...`);
  }
});

// Summary analysis
console.log('\n📈 REAL CONTENT QUALITY SUMMARY:');
console.log('='.repeat(80));

const qualityCounts = {};
qualityAnalyses.forEach(analysis => {
  qualityCounts[analysis.qualityComparison] = (qualityCounts[analysis.qualityComparison] || 0) + 1;
});

console.log('\n📊 QUALITY COMPARISON:');
Object.entries(qualityCounts).forEach(([comparison, count]) => {
  console.log(`- ${comparison}: ${count}/${qualityAnalyses.length}`);
});

// Count specific issues
const issueCounts = {};
qualityAnalyses.forEach(analysis => {
  analysis.chatBotIssues.forEach(issue => {
...
console.log('\n🎯 KEY FINDINGS:');
console.log('1. Questions where ChatBot returns irrelevant content');
console.log('2. Questions where ChatBot returns article links instead of direct answers');
console.log('3. Questions where ChatBot is overly verbose');
console.log('4. Overall content quality patterns');

// Save detailed analysis
const analysisPath = path.resolve(__dirname, "results", `real-content-quality-analysis-${Date.now()}.json`);
fs.writeFileSync(analysisPath, JSON.stringify(qualityAnalyses, null, 2));
