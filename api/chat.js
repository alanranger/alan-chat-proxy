...
async function findBestProductForEvent(client, firstEvent, preloadProducts = []) {
  ...
  const needLoc = location.length ? location : []; // require a location match if we have one

  // 1) Try among any preloaded products (same host favored)
  let best = null, bestScore = -1;
  for (const p of preloadProducts || []) {
    const hayOK = !needLoc.length || needLoc.some(l => (p.title || "").toLowerCase().includes(l) || (pickUrl(p) || "").toLowerCase().includes(l));
    if (!hayOK) continue;
    let s = overlapScore(refTokens, pickUrl(p), p.title);
    if (sameHost(p, firstEvent)) s += 0.08;
    if (/(workshop)/.test((p.title || "").toLowerCase())) s += 0.04;
    if (s > bestScore) { best = p; bestScore = s; }
  }
  if (best && bestScore >= 0.18) return best; // lenient but location-gated

  // 2) Query products table again with strict OR on tokens, require location hit
  const orParts = uniq([...Array.from(refTokens)]).slice(0, 12)
    .flatMap(t => [`title.ilike.%${t}%`, `page_url.ilike.%${t}%`, `description.ilike.%${t}%`, `raw->>metaDescription.ilike.%${t}%`, `raw->meta->>description.ilike.%${t}%`])
    .join(",");
  let fallback = [];
  if (orParts) {
    const { data } = await client.from("page_entities").select(SELECT_COLS).eq("kind","product").or(orParts).order("last_seen",{ascending:false}).limit(40);
    fallback = data || [];
  }

  // keep only those that mention the location (when we have one)
  const locFiltered = !needLoc.length ? fallback : fallback.filter(p => {
    const hay = ((p.title || "") + " " + (pickUrl(p) || "") + " " + (p.description || "")).toLowerCase();
    return needLoc.some(l => hay.includes(l));
  });

  best = null; bestScore = -1;
  for (const p of locFiltered) {
    let s = overlapScore(refTokens, pickUrl(p), p.title);
    if (sameHost(p, firstEvent)) s += 0.08;
    if (/(workshop)/.test((p.title || "").toLowerCase())) s += 0.04;
    if (s > bestScore) { best = p; bestScore = s; }
  ...
  // 3) As a last resort, fall back to a synthetic product block that points to the event (but we will still keep Book Now pointing to product only when we actually found one)
  return null;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (firstEvent) {
      ...
      // 1) Match a product for the first event (location-gated, host-favoured)
      const matched = await findBestProductForEvent(client, firstEvent, rankedProducts);

      // 2) If matched, optionally enrich price from clean view (same URL only)
      if (matched) {
        ...
        // 3) If no product could be found, DO NOT impersonate a product. Show advice/products below, pills won't include Book Now.
        featuredProduct = null;
      }

      // Put the featured product at the top of the product list (if found)
      if (featuredProduct) {
