...
// Version: v0.9.60-unified-pricing
...
/* ================= Utilities ================= */
const SELECT_COLS = "id, kind, title, page_url, source_url, last_seen, location, date_start, date_end, price, price_gbp, description, raw";
...
/* ================= Clean-price view (fallback enrichment) ================= */
...
/** Prefer canonical price/availability from materialized view v_products_unified. */
async function enrichFromUnifiedView(client, product) {
  const pUrl = pickUrl(product);
  if (!pUrl) return null;

  const canon = baseUrl(pUrl);
  // Exact / case-insensitive equality first
  let { data: rows, error } = await client
    .from("v_products_unified")
    .select("product_url, display_price_gbp, availability_status, location_hint")
    .eq("product_url", canon)
    .limit(1);
  if (error) throw error;

  let row = rows?.[0] || null;
  if (!row) {
    // Fallback to a contains match on the slug (safe – index on lower(product_url) exists)
    const slug = canon.split("/").slice(-1)[0];
    if (slug) {
      const { data: likeRows } = await client
        ...
        .limit(5);
      row = (likeRows || []).find(r => baseUrl(r.product_url) === canon) || (likeRows || [])[0] || null;
    }
  }
  if (!row) return null;

  return {
    price_gbp: (row.display_price_gbp != null ? Number(row.display_price_gbp) : null),
    availability_status: row.availability_status || null,
    location_hint: row.location_hint || null,
    matched_url: row.product_url
  };
}
...
function buildProductPanelMarkdown(prod) {
  ...
  const bullets = [];
  if (block.location || prod.location_hint) bullets.push(`- **Location:** ${block.location || prod.location_hint}`);
  ...
  try {
    ...
    if (firstEvent) {
      ...
      if (matched && strictlyMatchesEvent(matched, firstEvent, subtype)) {
        ...
        // === Enrich from unified materialized view (preferred) ===
        try {
          const uni = await enrichFromUnifiedView(client, featuredProduct);
          if (uni) {
            if (Number(uni.price_gbp) > 0) featuredProduct.price_gbp = Number(uni.price_gbp);
            if (uni.availability_status) featuredProduct.availability_status = uni.availability_status;
            if (uni.location_hint && !featuredProduct.location) featuredProduct.location_hint = uni.location_hint;
          } else {
            // Fallback to clean-price view if unified didn't match
            ...
            const found = cleanRows.find(r => baseUrl(r.url) === baseUrl(pickUrl(featuredProduct)));
            if (found && Number(found.price_gbp) > 0) featuredProduct.price_gbp = Number(found.price_gbp);
          }
        } catch {
          // Last-resort: do nothing if enrichment fails
        }
      }
    ...
    const structured = {
      ...
      products: (rankedProducts || []).map((p) => {
        ...
        return {
          ...
          location: p.location, location_hint: p.location_hint || null,
    ...
    const debug = {
      ...
      featured_product: featuredProduct ? {
        ...
        availability: featuredProduct.availability_status || null,
