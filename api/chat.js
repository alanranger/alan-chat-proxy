...
function analyzeDataAttributes(events, product, context) {
  ...
  if (events && events.length > 0) {
    ...
    const hasWorkshopQuery = queryLower.includes('workshop');
    const hasCourseQuery = queryLower.includes('course') || queryLower.includes('class');
...
function isCompletelyIrrelevant(qualityIndicators) {
  return (
    !qualityIndicators.hasDirectAnswer && 
    !qualityIndicators.hasRelevantEvents && 
    !qualityIndicators.hasRelevantArticles &&
    qualityIndicators.responseAccuracy < 0.3
  );
}

// Helper function to check for perfect response
function isPerfectResponse(qualityIndicators) {
  return (
    qualityIndicators.hasDirectAnswer && 
    qualityIndicators.hasRelevantEvents && 
    qualityIndicators.hasRelevantArticles && 
    qualityIndicators.hasActionableInfo &&
    qualityIndicators.responseCompleteness >= 0.9 &&
    qualityIndicators.responseAccuracy >= 0.9
  );
}

// Helper function to check for nearly perfect response
function isNearlyPerfectResponse(qualityIndicators) {
  return (
    qualityIndicators.hasDirectAnswer && 
    (qualityIndicators.hasRelevantEvents || qualityIndicators.hasRelevantArticles) &&
    qualityIndicators.responseCompleteness >= 0.8 &&
    qualityIndicators.responseAccuracy >= 0.8
  );
}

// Helper function to check for excellent event response
function isExcellentEventResponse(qualityIndicators) {
  return (
    qualityIndicators.hasDirectAnswer && 
    qualityIndicators.hasRelevantEvents &&
    qualityIndicators.responseCompleteness >= 0.3 &&
    qualityIndicators.responseAccuracy >= 0.6
  );
}

// Helper function to check for very good response
function isVeryGoodResponse(qualityIndicators) {
  return (
    qualityIndicators.hasDirectAnswer && 
    qualityIndicators.responseCompleteness >= 0.4 &&
    qualityIndicators.responseAccuracy >= 0.4
  );
}

// Helper function to check for good response
function isGoodResponse(qualityIndicators) {
  return (
    qualityIndicators.hasDirectAnswer || 
    qualityIndicators.hasRelevantEvents || 
    qualityIndicators.hasRelevantArticles
...
// Helper function to check for poor response
function isPoorResponse(qualityIndicators) {
  return qualityIndicators.responseCompleteness >= 0.3;
}

// Helper function to determine confidence score based on quality indicators
function determineConfidenceScore(qualityIndicators) {
  if (isCompletelyIrrelevant(qualityIndicators)) {
    return { score: 0.10, reason: "FORCED TO 10% - Completely irrelevant response" };
  }
  if (isPerfectResponse(qualityIndicators)) {
    return { score: 1.0, reason: "Perfect (100%) - All quality indicators met" };
  }
  if (isNearlyPerfectResponse(qualityIndicators)) {
    return { score: 0.95, reason: "Nearly Perfect (95%) - Direct answer + supporting content" };
  }
  if (isExcellentEventResponse(qualityIndicators)) {
    return { score: 0.95, reason: "Nearly Perfect (95%) - Excellent event response with relevant events" };
  }
  if (isVeryGoodResponse(qualityIndicators)) {
    return { score: 0.75, reason: "Very Good (75%) - Direct answer + good completeness/accuracy" };
  }
  if (isGoodResponse(qualityIndicators)) {
    return { score: 0.50, reason: "Good (50%) - Some useful content found" };
  }
  if (isPoorResponse(qualityIndicators)) {
    return { score: 0.30, reason: "Poor (30%) - Limited useful content" };
  }
  return { score: 0.10, reason: "Very Poor (10%) - Little to no useful content" };
}
...
function finalizeConfidence(query, context) {
  ...
  console.log(`   isCompletelyIrrelevant: ${isCompletelyIrrelevant(context.qualityIndicators)}`);
  
  const result = determineConfidenceScore(context.qualityIndicators);
  confidenceScore = result.score;
  console.log(`🎯 SELECTED: ${result.reason}`);
...
function calculateEntityConfidence(relevantEntities, chunks, results) {
    ...
    if (qualityScore >= 0.8) {
      results.confidence = 1.0; // 100% - Perfect
