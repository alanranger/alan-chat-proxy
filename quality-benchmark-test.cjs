...
async function runQualityBenchmark() {
  ...
  console.log(`✅ Tests Passed: ${results.filter(r => r.quality.overall >= 70).length}/${totalTests}`);
  console.log(`❌ Tests Failed: ${results.filter(r => r.quality.overall < 70).length}/${totalTests}`);
