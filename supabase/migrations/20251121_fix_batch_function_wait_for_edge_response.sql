-- Fix light_refresh_batch_with_regression_test to wait for Edge Function response
-- The function was calling the Edge Function but not waiting for it to complete
...
BEGIN
  ...
  -- Call Edge Function and WAIT for response
  BEGIN
    SELECT net.http_post(
    ...
    -- Wait for the Edge Function to complete (up to 5 minutes)
    v_http_response := wait_for_http_response(v_request_id, 300);
    IF NOT (v_http_response->>'success')::boolean THEN
      RAISE WARNING 'Light refresh Edge Function failed (batch %): status_code=%, error=%',
        p_batch,
        v_http_response->>'status_code',
        v_http_response->>'error';
    END IF;
...
COMMENT ON FUNCTION light_refresh_batch_with_regression_test(integer, boolean, boolean) IS 
  'Refreshes a batch of URLs. Now waits for Edge Function response before continuing. Chains to next batch if p_chain_next = true.';
