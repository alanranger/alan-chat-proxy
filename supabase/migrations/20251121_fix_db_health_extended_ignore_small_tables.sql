-- Fix db_health_extended to ignore very small tables with high dead row percentages
-- Small tables like regression_test_results (4.7MB) shouldn't drive the risk score
-- Only consider tables that are actually significant in size (>= 10MB) for bloat risk
...
BEGIN
  ...
  FROM pg_stat_user_tables
  WHERE pg_total_relation_size(relid) >= 10485760;  -- >= 10MB (was 1MB)
...
COMMENT ON FUNCTION public.db_health_extended() IS 
  'Extended database health check. Only considers tables >= 10MB for bloat risk calculation to avoid small tables driving the risk score.';
