-- Fix reltuples column error - pg_stat_all_tables doesn't have reltuples, need to join with pg_class
...
BEGIN
  ...
  SELECT
    ...
    pg_total_relation_size(st.relid) AS size_before_bytes,
    COALESCE(st.n_dead_tup, 0) AS dead_before,
    COALESCE(st.n_dead_tup, 0)
      * GREATEST(
          (pg_total_relation_size(st.relid)
             / GREATEST(COALESCE(c.reltuples, st.n_live_tup, 1), 1)),
          1
    ...
    est_bloat_after_bytes =
      ...
      * GREATEST(
          (after_stats.size_after_bytes
             / GREATEST(COALESCE(after_stats.reltuples, after_stats.n_live_tup, 1), 1)),
          1
        )
  FROM (
    SELECT
      st.relname AS table_name,
      pg_total_relation_size(st.relid) AS size_after_bytes,
      COALESCE(st.n_dead_tup, 0) AS dead_after,
      COALESCE(c.reltuples, st.n_live_tup, 1) AS reltuples,
