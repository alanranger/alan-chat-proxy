-- Fix run_database_maintenance_with_stats to use pg_class.reltuples (column exists) instead of pg_stat columns
-- Safe to rerun: creates or replaces the function only.
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      GREATEST(c.reltuples, 1) AS reltuples
    FROM pg_stat_all_tables st
    JOIN pg_class c ON c.oid = st.relid
    WHERE st.schemaname = 'public'
      AND st.relname = ANY(v_tables)
