...
async function findExposureArticles() {
  console.log('\n=== Searching for exposure/bracketing/histogram articles ===\n');
  
  const terms = ['exposure', 'bracket', 'histogram', 'dynamic range'];
  
  for (const term of terms) {
    const { data, error } = await client
      .from('articles')
      .select('id, title, page_url')
      .or(`title.ilike.%${term}%,page_url.ilike.%${term}%`)
      .limit(5);
...
findExposureArticles().catch(console.error);
