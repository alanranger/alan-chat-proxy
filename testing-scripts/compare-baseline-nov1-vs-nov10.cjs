#!/usr/bin/env node
/**
 * Compare Nov 1 baseline with Nov 10 test results (after JSON-LD improvements)
 */
...
// Current test from Nov 10 (deployed)
const currentPath = path.join(resultsDir, 'deployed-analytics-test-2025-11-10T15-58-09-401Z.json');

console.log('📊 COMPARING BASELINE (Nov 1) vs CURRENT (Nov 10 - After JSON-LD Improvements)\n');
...
const outputPath = path.join(resultsDir, `baseline-comparison-nov1-vs-nov10-${Date.now()}.json`);
