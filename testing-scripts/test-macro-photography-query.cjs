#!/usr/bin/env node
/**
 * Test "what is macro photography" query - investigate regression (12 -> 3 articles)
 */
...
const query = "what is macro photography";
console.log("=".repeat(80));
console.log("INVESTIGATION: 'what is macro photography'");
console.log("=".repeat(80));
console.log(`\nQuery: "${query}"`);
console.log(`Regression: 12 articles (baseline) -> 3 articles (current)\n`);
...
if (detection.hasEquipmentKeyword) {
  console.log(`   ✅ Equipment keyword detected`);
} else {
  console.log(`   ❌ Equipment keyword NOT detected`);
  console.log(`   Note: "macro" is in genreKeywords, not equipmentKeywords`);
}
console.log();
...
console.log(`   - "macro" is a single word, so filterArticleKeywords should work`);
console.log(`   - "macro" is in genreKeywords, NOT equipmentKeywords`);
console.log(`   - So equipment keyword filtering should NOT apply`);
console.log(`\n❓ QUESTIONS:`);
console.log(`   - Why did baseline have 12 articles but current only has 3?`);
console.log(`   - Is this the same recency boost issue?`);
console.log(`   - Are assignment articles replacing proper articles?`);
