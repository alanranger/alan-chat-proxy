...
// Test results from our comparison (only the ones we tested)
const testResults = {
...
console.log('📊 COMPLETE CONFIDENCE SCORING COMPARISON');
console.log('========================================');
...
Object.keys(groupedQuestions).forEach(group => {
  ...
  groupedQuestions[group].forEach(question => {
    ...
    const originalScore = testData ? (testData.original * 100).toFixed(0) + '%' : 'Not tested';
    const currentScore = testData ? (testData.current * 100).toFixed(0) + '%' : 'Not tested';
    const currentDiff = testData ? ((testData.current * 100) - alanScore).toFixed(1) : 'N/A';
...
console.log('- "Not tested" = Need to run comparison for these questions');
