...
// Find the latest current baseline (just generated)
...
// Use the most recent as current, second most recent as baseline
const currentJsonFile = currentFiles[0];
const baselineJsonFile = currentFiles[1]; // Use second most recent as baseline

const currentJsonPath = path.join(resultsDir, currentJsonFile);
const baselineJsonPath = path.join(resultsDir, baselineJsonFile);
...
// Generate CSV
const outputPath = path.join(resultsDir, `side-by-side-40q-${Date.now()}.csv`);
