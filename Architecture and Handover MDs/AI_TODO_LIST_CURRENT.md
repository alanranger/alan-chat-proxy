...
| 13 Nov 2025 | **CSV Import Event Date Cleanup Fix** | ✅ Fixed critical issue where CSV import was not removing old event dates when re-importing CSVs with rescheduled dates. Added deletion logic to remove old csv_metadata entries for URLs before inserting new ones. Fixed foreign key constraint issue by nullifying page_entities.csv_metadata_id references before deletion. System now properly cleans up old dates from both csv_metadata and page_entities when CSVs are re-imported. Verified: old dates (2025-11-29) successfully removed, new dates (2026-05-30) correctly stored |
